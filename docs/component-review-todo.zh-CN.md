...
| 编号 / 优先级 | 问题与证据 | 修复方向与验收 |
| --- | --- | --- |
| REV-77 / P1 | 基类只输出 hidden，组件 display 规则覆盖原生隐藏 | 基类按 Visible 追加/移除隐藏样式；真实 Surface/ComboBox 可交互切换验证及渲染契约通过 |
| REV-78 / P1 | ComboBox 方向键滚页触发 CloseOnScroll，确认可能重复原生点击 | 主 module 精确阻止已处理键的默认动作；浏览器完成打开/导航/确认，不滚页、不重开 |
| REV-79 / P1 | Drawer/Popover 模态 trap 无条件响应 Esc | 使用当前 closeOnEscape 策略；动态切换回归及 Drawer 浏览器实测通过 |
| REV-80 / P1 | Rating 值变化后焦点仍在原星级 | C# 同步真实焦点、module 阻止默认滚动；浏览器 3→4 星焦点一致 |
| REV-81 / P1 | ComboBox、Radio/RadioGroup 漏掉字段禁用/必填/错误继承 | 合并 FormField 状态；渲染/动态恢复、残留点击拒绝与浏览器验收通过 |
| REV-82 / P2 | DateOnly.MaxValue 渲染及导航/预设越界 | 保留 42 格、越界空白、限定月份/日期运算；三个日期控件边界渲染与键盘回归通过 |
| REV-83 / P2 | Tooltip 只首次绑定，动态启禁用/方位产生陈旧关联 | 每次渲染重绑并仅移除自身描述 token；单元及浏览器动态验证通过 |
| REV-84 / P2 | Tab 覆盖基类 hidden、导航含隐藏项 | 过滤标签/导航、保留面板隐藏、动态刷新；同时修复焦点模块缺少初始化声明，浏览器真实焦点验证通过 |
| REV-85 / P2 | Accordion 禁用展开项保留空白高度且标题颜色正常 | 渲染态折叠/inert，不修改宿主 OpenKeys；标题/箭头用禁用 Token，浏览器高度归零 |
| REV-86 / P2 | ComboBox 空集合无法打开 EmptyContent | 允许打开空面板且无活动选项；空状态展示与键盘安全回归通过 |
| REV-87 / P2 | Popup 示例使用不存在的 CloseOnOverlayClick 参数 | 改为 CloseOnOutsideClick，并补 CloseOnEscape 交互开关 |
| REV-88 / P3 | Drawer/Popover 关闭直接消失 | 按现有 Token 退场，等待实际 CSS 完成后释放模态资源；重开/释放取消旧完成，reduced-motion 即时结束，单元和浏览器通过 |
| REV-89 / P3 | TimePicker 每次参数变化重复全量映射 | 限定 Min/Max 候选，按实例复用无委托且参数不变的映射；保留可变委托重算，以全量参考枚举校验非整除步长/小数秒边界 |

以上新增行为回归进入 `tests/quality-interactions.test.mjs` 与 `AeterniUI.ContractChecks`，CI 同步运行所有 Node 测试。浏览器检查覆盖 41 个示例页 × 两个视口/主题及重点交互，不代表真实读屏软件与触屏设备已穷尽验收。
