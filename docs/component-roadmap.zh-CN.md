...
| 选择态中性化（品牌面收窄） | 已完成 | 通过 | 把品牌色从「状态标识」收窄为「身份标识」：选择态与悬浮/按下反馈改用**无色 overlay**，品牌只保留在身份面与焦点环上。**动因**：原来选中块取品牌填充，把品牌从紫换绿会连带改掉「哪些东西被选中了」的视觉语言，而选择态与品牌身份无关；中性墨填充让同一份组件在多品牌宿主里保持稳定，同时天然满足容器/状态的 R=G=B 约束。**落地方式（不新增 Token）**：`--aeterni-state-background-hover/active/selected` 改为 `rgba(0,0,0,.06/.12/.09)`（浅）与 `rgba(255,255,255,.08/.16/.11)`（深，alpha 由 OKLab 等感知强度反解旧品牌染后取整阶梯），`--aeterni-state-color-*` 与 `--aeterni-state-background-checked` 改指既有的 `--aeterni-text-primary`，`--aeterni-state-border-hover/active` 取 `rgba(0,0,0,.26/.38)` 与 `rgba(255,255,255,.26/.34)`（静止态是 `border-strong` 的 `.18/.16`，必须更高才有可见台阶，已实测有台阶）；`--aeterni-color-brand-soft` 与品牌色脱钩改为独立 `color-mix`（它只被 Avatar 与 `button-intent-default-soft` 消费，不再受 overlay 变化牵连）。三套模式块（浅色、深色、`prefers-color-scheme` 兜底）同构改写。**焦点环保持品牌**：`state-color-focus`/`state-border-focus`/`focus-color`/`text-link` 未动，对比度门禁 pair「`state-color-focus × bg-surface`」仍按 GRAPHIC_FLOOR 3 通过。**组件层 9 处**：Checkbox（勾选/不确定块、悬浮边框）、Radio（内圆点、选中环、悬浮边框）、Switch（选中轨道 + 新增选中态旋钮翻 `--aeterni-text-inverse`，因为深色下轨道是近白色）、Segmented（滑块）、Tabs（选中文字与 2px 指示条）、Menu（选中项内阴影条/边框/图标）、DatePicker 与内部 DateCalendar（选中日期、今天标记）、Pagination 当前页（**超出原定名单，一并中性化**）。**顺带修掉一个坑**：中性化后已勾选控件的边框色等于填充色，原先 `:hover:not(:disabled)` 会在实心块上压出一圈亮边，三个开关类组件的悬浮选择器收紧为 `:hover:not(:disabled):not(:checked)`（Checkbox 另加 `:not(:indeterminate)`）。**验证**：`check-contrast.mjs` **99 项全 PASS**（最紧仍是 green/light `text-inverse × brand-default` 4.52:1，margin 1.004）；`check-css-comments.mjs` 48 文件 PASS；Token 选择器门禁为空；`dotnet build aeterni_ui.slnx` 0 警告 0 错误；浏览器端到端 3 品牌 × 浅/深逐条断言，checkbox 填充/开关轨道/开关旋钮/选中标签墨/指示条/当前页/滑块/选中边框/圆点/圆环/悬浮边框/选中日期/今天标记在**同一模式内三品牌全部相同**，而 Button 实心填充三品牌各不相同（`#795AD9`/`#1F883D`/`#C2410C`），焦点环与链接三品牌仍各自解析为品牌色；像素直读浅色选中块 `rgb(56,56,56)`、深色 `rgb(223,223,223)`，饱和度 0（填充承字实测 11.73:1 与 13.03:1）。规范侧新增 §5.3「品牌色面积」两行表格（身份面/焦点与链接 vs 必须中性化的选择态）与「悬浮态不得覆盖已勾选控件」硬规则，§5.4 填充档用途表与 §12 样式清单同步。**宿主可一行恢复品牌选中态**：在自己的样式表里重写 `--aeterni-state-background-checked` 即可，不需要改组件。 |
