...
状态：v0.1～v0.5 与第六轮质量收口已交付；v0.6 Toolbar、ToggleGroup、SplitButton、Breadcrumb、Stepper 已交付（5/5）；10.17.1 交付 List 活动项底色修复；v0.7 规划中。

## 10.16.0 Stepper 功能发布

- [x] 新增 `Stepper` / `StepperItem`，交付受控当前步骤、宿主显式完成/禁用状态、横纵向布局和位置 ARIA 语义。
- [x] 新增 `/components/stepper` 可操作示例、组件渲染契约和导航模块索引；不内置路由、异步编排或业务流程状态管理。
- [x] 版本属性统一为 10.16.0；同步当前功能、设计规范、项目索引、README 发布 tag 示例，并通过构建、文档、CSS 检查。
...
状态：Toolbar、ToggleGroup、SplitButton、Breadcrumb 与 Stepper 已交付（5/5）。
...
5. [x] `Stepper`：服务于线性流程，不把业务流程状态管理内置到组件库；提供受控当前步骤、显式完成/禁用状态和横纵向布局。
...
| v10.17.1 List 活动项底色修复 | 已完成 | 构建、渲染契约、List 键盘回归、CSS 注释、文档与品牌对比度门禁 | 把 `.is-active` 的活动项底色限定为列表持有键盘焦点时生效，修复多选与 `AllowClear` 下取消选中后遗留底色；`SetActive` 与 `aria-activedescendant` 未改动，键盘提示保留。 |
