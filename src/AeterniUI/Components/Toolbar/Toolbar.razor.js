...
const selector = 'button.aeterni-button, button.aeterni-icon-button';
...
function owned(state, button) {
    ...
    return button.closest('[role="toolbar"]') === state.host &&
...
function refresh(state) {
    ...
    state.observer.observe(host, { subtree: true, childList: true, attributes: true,
        attributeFilter: ['disabled', 'aria-disabled', 'aria-busy', 'hidden', 'inert', 'class', 'style', 'tabindex', 'role', 'aria-orientation'] });
    return enabled;
}
