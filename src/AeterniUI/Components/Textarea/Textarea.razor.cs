...
namespace AeterniUI.Components.Textarea;

public partial class Textarea : AeterniComponent
{
    [CascadingParameter]
    ...
    [Parameter]
    public TextareaResize Resize { get; set; } = TextareaResize.Vertical;
    ...
    private string ResizeClass => Resize switch
    {
        ...
        _ => throw new ArgumentOutOfRangeException(nameof(Resize), Resize, "Unknown textarea resize mode.")
    };

    private void ValidateParameters()
    {
        if (!Enum.IsDefined(Resize))
        {
            throw new ArgumentOutOfRangeException(nameof(Resize), Resize, "Unknown textarea resize mode.");
        }
    ...
    private static void AddAttribute(IDictionary<string, object> attributes, string name, string? value)
    {
